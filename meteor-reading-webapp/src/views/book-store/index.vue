    ...
    watch: {
        $route() {
            if (this.tagId !== this.$route.query.tagId) {
                if (this.$store.state.bookStore.cache[this.$route.query.tagId]) {
                    return this.$store.commit('bookStore/setDataFromCache', this.$route.query.tagId);
                }
                this.$store.dispatch('setLoading', true);
                this.fetchData(1).then(() => {
                    this.$store.dispatch('setLoading', false);
                    this.hackReset = true;
                    this.$store.dispatch('bookStore/setCurTagId', this.$route.query.tagId);
                });
            } else {
