...
export default {
    ...
    actions: {
        fetchNovelListData({ commit }, { page, status, tagId }) {
            return bookApi.fetchNovelList(page, tagId, status)
                .then((_) => commit('setNovelListData', {
                    tagId,
                    page,
                    items: _.data.data
                }));
    ...
    mutations: {
        setNovelListData(state, data) {
            ...
            if (tagId !== state.curTagId) {
                state.items = [];
            }

            initCache(state, tagId);
            ...
            // 缓存
            state.cache[tagId].items = state.items;
            state.cache[tagId].curPage = state.curPage;
        },
        initCache(state) {
